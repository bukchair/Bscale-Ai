...
export default async function handler(
  ...
  try {
    const accessToken = await getValidServiceAccessToken({ userId, service: 'search_console' });
    res.statusCode = 200;
    res.setHeader('content-type', 'application/json');
    ...
    res.statusCode = 400;
    res.setHeader('content-type', 'application/json');
    res.end(JSON.stringify({ message: error?.message || 'Search Console service is not connected.' }));
  }
}

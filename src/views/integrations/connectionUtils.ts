...
import { Sparkles, Zap, BrainCircuit, Megaphone, Facebook, Video, ShoppingCart, Store } from 'lucide-react';
import { Connection } from '../../contexts/ConnectionsContext';
import { ManagedGoogleAdsAccount } from './types';
...
export const normalizeMetaAccountId = (value: string) => String(value || '').replace(/^act_/i, '').trim();
...
export const brandStyles: Record<string, { bg: string, text: string, border: string, lightBg: string }> = {
